...
\title{Impact of Exchange Rate Pass-Through to South African Prices}
...
The SARB's Nominal Effective Exchange Rate\footnote{a detailed description of the methodology is available at https://www.resbank.co.za/content/dam/sarb/publications/quarterly-bulletins/quarterly-bulletin-publications/2020/10091/03Note-on-the-revision-of-South-Africas-nominal-and-real-effective-exchange-rate-indices.pdf}, which measures the external value of the Rand against a weighted basket of currencies is the preferred measure for the exchange rate\footnote{The USD-ZAR exchange rate is used as a robustness check}. Although currencies are weighted by partners' shares of imports and exports, it is preferred to the Dollar given the importance of other major economies like China, the UK and the EU as trade partners and has precedents in previous studies\footnote{See \textcite{mccarthy1999}, \textcite{Karoroetal2008},   \textcite{aronetal2012},\textcite{choudri2015exchange}, \textcite{kabundi2016exchange}}. Indexed values are log-differenced and flipped such that positive movements represent depreciations.

South African import data are characterised by various breaks which compromise its suitability: between 1980 and 2012, import prices formed part of producer price indices and were subdivided by Standard Industry Classification. After 2010, import prices were obtained from customs data using a classification system based on the Central Product Classification System, which aggregates goods together according to physical characteristics. This index was calculated as the arithmetic mean up until 2023, while a geometric mean was used from 2017 up to the time of writing \cite{StatsSA2025XUVI}. Tests were carried out to determine the compatibility between the different series. 

 The overlapping period between 2010 and 2013 also indicates that the two series 

 For producer and consumer prices, headline PPI and CPI figures are used. All price series are log-differenced. 

Results of diagnostic tests are provided in the \ref{sec:Appendix}. Series are found to be stationary according to Augmented Dickey-Fuller tests, thus justifying the use of the VAR model as opposed to a vector error correction model. Residuals from the VAR models display very low correlation between different variables and across lags. Normality tests indicate that our data displays skewness, with Jarque-Bera p-values being significant beyond 1\% for all variables. With this in mind, series are bootstrapped??????
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{irf_12lags.png}
    \caption{Impulse Response Functions with 12 Lags}
    \label{fig:irf_12lags}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{irf_endog_oil.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{adj_12lags.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{adj_endog_oil.png}
...
\subsection{Chow-Lin Disaggregation}
