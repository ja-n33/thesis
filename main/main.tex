...
where the coefficient $\theta_{VAR}^h$ captures the dynamic effect of a exchange rate shock propagated through \eqref{eq:reduced_form} recursively over $h$ periods. 
In addition, while standard VAR and LP models are sign-agnostic in that disturbances are assumed to have symmetric effects (for example, a 1\% appreciation has the exact opposite impact as a 1\% depreciation), 
  ...
  The tendency of AIC to pick out more parsimonous models with shorter lag lengths risks longer-horizon dynamics being omitted. 
  
  
  Oil shocks, for example, can have gradual or delayed impacts on economic variables, meaning that short lag lengths may miss these effects altogether \cite{EDELSTEIN2009766, hamilton-herrera}. While \textcite{mccarthy1999} and \textcite{hahn2003} opt for a year's worth of lags, lag order is restricted to a year. 
