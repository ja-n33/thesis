...
South African import data is characterised by various breaks which compromise its suitability: between 1999 and 2012, import prices formed part of producer price indices and were subdivided by Standard Industry Classification. After 2010, import prices were obtained from customs data using a classification system based on the Central Product Classification System, which aggregates goods together according to physical characteristics \cite{StatsSA2025XUVI}. This index was calculated as the arithmetic mean up until 2022, while a geometric mean was used from 2017 up to the time of writing. With no series spanning more than 150 periods, the VAR model would have more parameters to calculate than there are data points with 6 and 12 lags. Linking across the SIC total import index and customs-based total import index provides the most coherent option to extend the series, however across the overlapping period, they were found to have a Pearson correlation of 0.43, reflecting inconsistent measurement and calling for caution in using it. Given these issues, including these series may introduce more bias and variance into the model than simply omitting imports from the main specification. To the extent that higher import prices are positively correlated with exchange rates and subsequent prices, exchange rate shocks will have additional explanatory power attributed to them, resulting in higher-than-actual estimates (an upward bias). 
